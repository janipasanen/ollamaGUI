...
describe('/copy slash command (#279)', () => {
  it('copies the conversation as Markdown to the clipboard', async () => {
  ...
  it('shows a hint when the conversation is empty', async () => {
    ...
    render(<App />);
    fireEvent.change(screen.getByPlaceholderText('Message Ollama...'), { target: { value: '/copy' } });
    fireEvent.click(screen.getByRole('button', { name: 'Send message' }));
