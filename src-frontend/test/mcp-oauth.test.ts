...
// Seed a client credential (now stored via secretStore, not localStorage).
async function seedClient(serverId: string, clientId: string) {
  await secretStore.set('clients', JSON.stringify({ [serverId]: { client_id: clientId } }));
}
...
describe('HTTP MCP OAuth token attachment (#107)', () => {
  ...
  it('sends a stored, valid OAuth access token as a bearer', async () => {
    await tokenStore.save('h1', { access_token: 'tok1', token_type: 'Bearer', expires_at: Date.now() + 3_600_000 });
    authMetaStore.save('h1', { tokenEndpoint: 'https://auth.example.com/token' });
    await seedClient('h1', 'c1');
  ...
  it('refreshes an expired access token and uses the fresh one', async () => {
    await tokenStore.save('h1', { access_token: 'old', token_type: 'Bearer', refresh_token: 'r1', expires_at: Date.now() - 1_000 });
    authMetaStore.save('h1', { tokenEndpoint: 'https://auth.example.com/token' });
    await seedClient('h1', 'c1');
    ...
    expect((await tokenStore.load('h1'))?.access_token).toBe('fresh');
  });
